...
void DBSCAN(vector<Point2f> CENTERS,float Eps,int MinPts){//参数含义？
	vector<point> dataset;
	for(int alpha=0;alpha<CENTERS.size();alpha++){
		//point temp;
		//temp.x=CENTERS[alpha].x;
		//temp.y=CENTERS[alpha].y;
		dataset.push_back(point(CENTERS[alpha].x,CENTERS[alpha].y,0));
	}
	...
	//output
	//display  
	//cout<<corePoint.size()<<endl;
	cout<<len<<endl;
	/*for(int i=0;i<corePoint.size();i++){
		cout<<"第"<<i+1<<"个Core: "<<corePoint[i].x<<","<<corePoint[i].y<<","<<corePoint[i].cluster<<"\n";
	}*/
