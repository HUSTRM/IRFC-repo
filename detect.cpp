...
{//based on k-means
	...
	Mat lables;
	Mat center(3,1,samples.type());
	for(i=0;i<points.size();i++){
		samples.at<Point2f>(i)=points[i];
		}
	kmeans(samples,3,lables,TermCriteria( CV_TERMCRIT_EPS+CV_TERMCRIT_ITER, 10, 1.0),3,KMEANS_PP_CENTERS, center);
	if(!center.empty()){
		for(i=0;i<3;i++)
